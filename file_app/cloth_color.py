...
def cloth_color_filter(main_img, back_img):
    ...
    red[np.where(np.isin(parsed, [5, 6, 7, 9, 12], invert=True))] = main_img[np.where(
        np.isin(parsed, [5, 6, 7, 9, 12], invert=True))]
    red[parsed == 0] = back_img[parsed == 0]

    blue[np.where(np.isin(parsed, [5, 6, 7, 9, 12], invert=True))] = main_img[np.where(
        np.isin(parsed, [5, 6, 7, 9, 12], invert=True))]
    blue[parsed == 0] = back_img[parsed == 0]

    sky[np.where(np.isin(parsed, [5, 6, 7, 9, 12], invert=True))] = main_img[np.where(
        np.isin(parsed, [5, 6, 7, 9, 12], invert=True))]
    sky[parsed == 0] = back_img[parsed == 0]
